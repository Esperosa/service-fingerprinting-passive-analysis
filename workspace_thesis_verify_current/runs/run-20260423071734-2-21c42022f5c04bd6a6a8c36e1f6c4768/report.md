...
- ID běhu: `run-20260423071734-2-21c42022f5c04bd6a6a8c36e1f6c4768`
...
### Na službě 192.168.56.10/host-only je zvýšený výskyt timeout/retry provozu [střední / nízká jistota]

- Typ: `connection_timeout_burst`
- Cíl: 192.168.56.10/host-only
...
- Evidence:
  - service_key=192.168.56.10/host-only
...
### Korelace pro 192.168.56.10/host-only obsahuje události s nižší jistotou [nízká / nízká jistota]
...
- Evidence:
  - unexpected_traffic | ip-only | Pozorovan provoz na port 8443, ktery neni v aktivnim inventari jako otevrena sluzba.
...
### Služba 192.168.56.10/tcp/8080 představuje exponovanou správcovskou nebo interní plochu [střední / vysoká jistota]

- Typ: `management_surface_exposure`
- Cíl: 192.168.56.10/tcp/8080
- Důvod: Na otevřeném portu byla zjištěna služba typická pro správu, sdílení nebo interní databázový provoz; bez kontextu použití jde o kandidáta k ověření expozice.
- Doporučení: Ověřit, zda má být služba dostupná z daného segmentu, a případně omezit přístup segmentací, firewall pravidly nebo přesunem za VPN či správní zónu.
- Evidence:
  - service_key=192.168.56.10/tcp/8080
  - service_name=http | product=nginx | version=1.24.0

### Mezi běhy se objevila nově exponovaná služba 192.168.56.10/host-only [nízká / vysoká jistota]

- Typ: `new_exposed_service`
- Cíl: 192.168.56.10/host-only
- Důvod: Diff potvrdil změnu inventáře proti předchozímu běhu; nová služba může představovat legitimní změnu i nové riziko.
- Doporučení: Ověřit důvod zpřístupnění služby, zkontrolovat vlastnictví změny, autorizaci a případně doplnit monitorovací nebo hardening opatření.
- Evidence:
  - change_type=nova_sluzba
  - after=n/a|Korelace pouze na uroven hosta||host-only

### Mezi běhy se objevila nově exponovaná služba 192.168.56.10/tcp/8080 [vysoká / vysoká jistota]
...
- Evidence:
  - change_type=nova_sluzba
  - after=open|nginx|1.24.0|http

### Mezi běhy se objevila nově exponovaná služba 192.168.56.40/tcp/80 [střední / vysoká jistota]

- Typ: `new_exposed_service`
- Cíl: 192.168.56.40/tcp/80
- Důvod: Diff potvrdil změnu inventáře proti předchozímu běhu; nová služba může představovat legitimní změnu i nové riziko.
- Doporučení: Ověřit důvod zpřístupnění služby, zkontrolovat vlastnictví změny, autorizaci a případně doplnit monitorovací nebo hardening opatření.
- Evidence:
  - change_type=nova_sluzba
...
### Na službě 192.168.56.10/host-only byla detekována špička rychlosti paketů [střední / nízká jistota]
...
### Služba 192.168.56.10/host-only vykazuje riziko přetížení a degradace [vysoká / nízká jistota]
...
- Evidence:
  - connection_timeout_burst:192.168.56.10/host-only
  - packet_rate_spike:192.168.56.10/host-only
  - service_overload_risk:192.168.56.10/host-only
...
- Evidence:
  - traffic=connection_timeout_burst target=192.168.56.10/host-only
  - traffic=packet_rate_spike target=192.168.56.10/host-only
  - traffic=service_overload_risk target=192.168.56.10/host-only
...
- Evidence:
  ...
  - management_surface_exposure:192.168.56.10/tcp/8080
  - new_exposed_service:192.168.56.10/tcp/8080
  - new_exposed_service:192.168.56.40/tcp/80
  - packet_rate_spike:192.168.56.10/host-only
...
### Znovu ověřit nově exponované služby [střední]

- Typ: `spawn-agent:exposure-retest-agent`
- Cíl: 192.168.56.10/host-only
- Důvod: Nová služba proti baseline může být legitimní změna i riziko. Agent má zopakovat bezpečný aktivní dotaz, zkontrolovat vlastnictví změny a propojit výsledek s pasivním provozem.
- Nástroje: nmap-followup, service-banner, controller-context, change-review
- Evidence:
  - new_exposed_services=3
...
- Evidence:
  - connection_timeout_burst:192.168.56.10/host-only
  - correlation_uncertainty:192.168.56.10/host-only
...
- Typ: `automation`
- Souhrn: Context agent sjednotil 4 aktiv a 3 vazeb z autorizovaných zdrojů.
- Evidence:
...
- Zdroj: `correlator`
- Typ: `automation`
- Souhrn: Korelační agent navázal 15 nálezů a 12 doporučených kroků.
- Evidence:
  - audit_findings=0
...
- Evidence:
  - plaintext=192.168.56.20/tcp/21
  - plaintext=192.168.56.20/tcp/23
  - source_action=validation:credential-exposure-agent:run-20260423071734-2-21c42022f5c04bd6a6a8c36e1f6c4768
...
- Evidence:
  - cve=CVE-2021-3618
  - cve=CVE-2023-38408
  - source_action=validation:cve-proof-agent:run-20260423071734-2-21c42022f5c04bd6a6a8c36e1f6c4768

### Agent agent:exposure-retest-agent [spawned]

- Zdroj: `agent:exposure-retest-agent`
- Typ: `automation`
- Souhrn: Dynamický agent exposure-retest-agent vznikl z rozhodovací vrstvy, protože běh má konkrétní důkaz a ověřovací úkol.
- Evidence:
  - new_exposed_services=3
  - source_action=validation:exposure-retest-agent:run-20260423071734-2-21c42022f5c04bd6a6a8c36e1f6c4768
...
- Evidence:
  ...
  - packet_rate_spike:192.168.56.10/host-only
  - service_overload_risk:192.168.56.10/host-only
  - source_action=decision:identity-verifier:run-20260423071734-2-21c42022f5c04bd6a6a8c36e1f6c4768
...
- Evidence:
  - plaintext_management_protocol:192.168.56.20/tcp/21
  - plaintext_management_protocol:192.168.56.20/tcp/23
  - source_action=decision:plaintext-passive-hunter:run-20260423071734-2-21c42022f5c04bd6a6a8c36e1f6c4768
...
- Evidence:
  ...
  - packet_rate_spike:192.168.56.10/host-only
  - service_overload_risk:192.168.56.10/host-only
  - source_action=decision:traffic-forensics:run-20260423071734-2-21c42022f5c04bd6a6a8c36e1f6c4768
...
- Evidence:
  ...
  - traffic=packet_rate_spike target=192.168.56.10/host-only
  - traffic=service_overload_risk target=192.168.56.10/host-only
  - source_action=validation:traffic-forensic-agent:run-20260423071734-2-21c42022f5c04bd6a6a8c36e1f6c4768
...
- Evidence:
  - high_risk_cve_exposure:192.168.56.30/tcp/22
  - source_action=decision:vuln-intel-verifier:run-20260423071734-2-21c42022f5c04bd6a6a8c36e1f6c4768
...
- Evidence:
  - web_targets=4
  - active_checks=0
  - source_action=validation:web-pentest-agent:run-20260423071734-2-21c42022f5c04bd6a6a8c36e1f6c4768
...
- Zdroj: `intel`
- Typ: `automation`
- Souhrn: Intel agent zpracoval 4 externích matchů, provider demo a veřejný stack používá Vulners jen jako volitelný doplněk.
- Evidence:
  - cves=2
...
- Zdroj: `inventory`
- Typ: `automation`
- Souhrn: Inventarizační agent zpracoval 4 hostů a 8 služeb.
- Evidence:
  - followup=false
  - forensic=false
...
- Zdroj: `planner`
- Typ: `automation`
- Souhrn: Plánovač seřadil 2 prioritních cílů a rozdělil běh do kroků inventář -> zpřesnění -> korelace -> intel.
- Evidence:
...
- Zdroj: `remediation`
- Typ: `automation`
- Souhrn: Remediation agent převádí ověřené nálezy na 12 doporučených kroků s důkazy.
- Evidence:
  - triage_actions=12
...
- Zdroj: `reporter`
- Typ: `automation`
- Souhrn: Report agent sjednotil 7 lane, 15 nálezů a 12 doporučených kroků.
- Evidence:
  - run_id=run-20260423071734-2-21c42022f5c04bd6a6a8c36e1f6c4768
...
- `192.168.56.10/host-only` | host-only | stav=n/a | priorita=nizka | skóre=0
...
- `192.168.56.10/tcp/80` | http | stav=open | priorita=nizka | skóre=0
  - produkt: Apache httpd 2.4.58
  - CPE: cpe:2.3:a:apache:http_server:2.4.58:*:*:*:*:*:*:*
- `192.168.56.10/tcp/443` | https | stav=open | priorita=nizka | skóre=0
  - produkt: Apache httpd 2.4.58
  - CPE: cpe:2.3:a:apache:http_server:2.4.58:*:*:*:*:*:*:*
- `192.168.56.10/tcp/8080` | http | stav=open | priorita=nizka | skóre=0
  - produkt: nginx 1.24.0
  - CPE: cpe:2.3:a:nginx:nginx:1.24.0:*:*:*:*:*:*:*
  - Události: suricata_alert
### 192.168.56.20
...
- `192.168.56.40/tcp/80` | http | stav=open | priorita=nizka | skóre=0
  - produkt: nginx 1.24.0
  - CPE: cpe:2.3:a:nginx:nginx:1.24.0:*:*:*:*:*:*:*
  - Události: http_basic_without_tls
## Diff

- Referenční běh: `run-20260423071733-1-5cc34c65d58f4ad1a80484f274f3bd31`
- Nové hosty: 192.168.56.40
- Změněné služby: 192.168.56.10/host-only, 192.168.56.10/tcp/443, 192.168.56.10/tcp/80, 192.168.56.10/tcp/8080, 192.168.56.40/tcp/80, 192.168.56.30/host-only
